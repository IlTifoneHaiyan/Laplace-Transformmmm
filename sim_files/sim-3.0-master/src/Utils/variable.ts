...
type costTypes = SuperExponentialCost | ExponentialCost | CompositeCost | StepwiseCost;
...
export class SuperExponentialCost {
  cost: number;
  initialCostInc: number;
  increment: number;
  /**
   * SuperExponentialCost constructor - costInc increases by a fixed amount every purchase
   * @param {number} base BaseCost of the variable
   * @param {number} initialCostInc Initial cost increase
   * @param {number} increment Amount the initial cost increase increases by, per level
   * @param {boolean} log2 States whether the cost increase is log2 or not - optional, default: false
   */
  constructor(base: number | string, initialCostInc: number | string, increment: number | string, log2: boolean | null = false) {
    this.cost = parseValue(String(base));
    this.initialCostInc = parseValue(String(initialCostInc));
    this.increment = parseValue(String(increment));
    if (log2) this.initialCostInc = Math.log10(2) * 10 ** this.initialCostInc;
  }
  getCost(level: number) {
